...
if __name__ == '__main__':
    ...
    add_data = [BitCoin(datetime=row[0],open=float(getattr(row[1],'open')),high=float(getattr(row[1],'high')),low=float(getattr(row[1],'low')),close=float(getattr(row[1],'close')),volume=float(getattr(row[1],'volume')),adj_close=float(getattr(row[1],'adj_close')))  for row in data_rows]
